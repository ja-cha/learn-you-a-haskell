...
import           Utils      (addThreeNumbers, factorial', flip',
                             ...
                             quicksort, twoRowComposition, zipWithLambda)
...
main =
  hspec $ do
    ...
    describe ">>> Factorial" $ do
      it "factorial' 5 shouldBe 120" $ do factorial' 5 `shouldBe` 120
    describe ">>> Flip" $ do
      it "3 - 2 `shouldBe` flip' (-) 2 3 " $ do 3 - 2 `shouldBe` flip' (-) 2 3
    describe ">>> Lambda" $ do
    ...
    describe ">>> Long Chains" $ do
      it
        "[2.0,5.0,10.0,20.0,25.0] `shouldBe` zipWithLambda (\\l r -> (l * 10) / 5)" $ do
