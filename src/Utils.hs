module Utils (quicksort, largestDivisible, factorial', flip', addThreeNumbers, numChainsWhere, zipWithLambda, twoRowComposition, oddSquareSumComposition, oddSquareSumBindings) where
...
numChainsWhere paf  = length (filter predicate (map chain [1..100]))  
    where predicate xs = paf (length xs)
